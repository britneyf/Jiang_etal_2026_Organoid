...
# Non-Tumor is normal
#adata_subset = adata[adata.obs['Tumor Status'].isin(['Non-Tumor'])].copy()

# Subset the data for 'Metastasis' and 'Primary Tumor'
adata_subset = adata[adata.obs['Tumor Status'].isin(['Metastasis', 'Primary Tumor'])].copy()
...
# **Filter out genes with all zero counts across all cells**
...
hs_results.to_csv(hs_dir + 'hotspot.patient.tumor.grouped.informative_genes.tsv', sep='\t')
...
local_correlations.to_csv(hs_dir + 'hotspot.patient.tumor.grouped.local_correlations.tsv', sep='\t')

# Create modules
modules = hs.create_modules(min_gene_threshold=50, core_only=True, fdr_threshold=0.05)
modules.to_csv(hs_dir + 'hotspot.patient.tumor.grouped.modules.tsv', sep='\t')

# Calculate module scores
module_scores = hs.calculate_module_scores()
module_scores.to_csv(hs_dir + 'hotspot.patient.tumor.grouped.module_scores.tsv', sep='\t')

# Plot local correlations
hs.plot_local_correlations()
plt.savefig(hs_dir + 'hotspot.patient.tumor.grouped.local_correlation_plot.png')