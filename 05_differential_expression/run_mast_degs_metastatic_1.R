...
adata_metastatic = sc$read_h5ad('/data/chanjlab/CRC_ZFP36L2.092023/Organoid/output_new/postprocess_adata/adata.metastatic.h5ad')
...
rownames(counts) <- adata_metastatic$var_names$to_list()
colnames(counts) <- adata_metastatic$obs_names$to_list()

# Make a single cell assay object
sca <- FromMatrix(counts, adata_metastatic$obs, adata_metastatic$var)
...
# Export data
write.csv(as.data.frame(fcHurdleSig), sprintf("%s/%s.csv", '/data/chanjlab/CRC_ZFP36L2.092023/Organoid/output_new/mast/', 'metastatic_1_zfpexp'), quote=FALSE)

# Export data
#write.csv(as.data.frame(fcHurdle), sprintf("%s/%s.csv", '/data/chanjlab/CRC_ZFP36L2.092023/Organoid/output_new/mast/mast.022124/', 'metastatic_1_zfpexp'), quote=FALSE)
