...
adata_metastatic = sc$read_h5ad('/data/chanjlab/CRC_ZFP36L2.092023/Organoid/input/degs/metastatic_tumor.h5ad')
...
coldata <- adata_metastatic$obs[,c("Culture_Media", "ZFP_Expression")]
...
dds_1 <- DESeqDataSetFromMatrix(
  countData = counts,
  colData = coldata,
  design = ~ Culture_Media + ZFP_Expression + ZFP_Expression:Culture_Media 
)
...
result_file <- paste(directory_path, "/metastatic_zfpexp_cmint_results_v2.csv", sep="")

# Specify the file path where you want to save the results
#result_file <- paste(directory_path, "/metastatic_zfpexp_cmint_results.csv", sep="")

# Write the results table to a CSV file
write.csv(as.data.frame(res_1), file = result_file)

# Save the DESeqDataSet object
saveRDS(dds_1, file = paste(directory_path, "/metastatic_zfpexp_cmint_dds_v2.rds", sep=""))

# Save the DESeq results
saveRDS(results(dds_1), file = paste(directory_path, "/metastatic_zfpexp_cmint_results_v2.rds", sep=""))