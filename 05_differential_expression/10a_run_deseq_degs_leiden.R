...
# Read in the adata for metastatic tumor site
adata_metastatic = sc$read_h5ad('/data/chanjlab/CRC_ZFP36L2.092023/Organoid/output_new/postprocess_adata/adata.combined.postprocess.leiden.h5ad')

# Get the counts matrix
counts = as.matrix(adata_metastatic$X)
...
rownames(counts) <- adata_metastatic$var_names
colnames(counts) <- adata_metastatic$obs_names

# Define the column data
coldata <- adata_metastatic$obs[,c("ZFP_Expression", "leiden_res_0.1")]
...
coldata$leiden_res_0.1 <- factor(coldata$leiden_res_0.1)
...
dds_1 <- DESeqDataSetFromMatrix(
  countData = counts,
  colData = coldata,
  design = ~ leiden_res_0.1 + ZFP_Expression 
)
...
result_file <- paste(directory_path, "/deseq_results_leiden.csv", sep="")

# Specify the file path where you want to save the results
#result_file <- paste(directory_path, "/metastatic_zfpexp_results.csv", sep="")

# Write the results table to a CSV file
write.csv(as.data.frame(res_1), file = result_file)

# Save the DESeqDataSet object
saveRDS(dds_1, file = paste(directory_path, "/deseq_results_leiden.rds", sep=""))

# Save the DESeq results
saveRDS(results(dds_1), file = paste(directory_path, "/deseq_results_leiden.rds", sep=""))