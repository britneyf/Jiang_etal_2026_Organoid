...
result_file <- paste(directory_path, "/primary_zfpexp_cmint_results.csv", sep="")

# Write the results table to a CSV file
write.csv(as.data.frame(res_1), file = result_file)

# Save the DESeqDataSet object
saveRDS(dds_1, file = paste(directory_path, "/primary_zfpexp_cmint_dds.rds", sep=""))

# Save the DESeq results
saveRDS(results(dds_1), file = paste(directory_path, "/primary_zfpexp_cmint_results.rds", sep=""))