...
result_file <- paste(directory_path, "/sc_zfpexp_cmint_lrt_results.csv", sep="")

# Write the results table to a CSV file
write.csv(as.data.frame(res_1), file = result_file)

# Save the DESeqDataSet object
saveRDS(dds_lrt, file = paste(directory_path, "/sc_zfpexp_cmint_lrt_dds.rds", sep=""))

# Save the DESeq results
saveRDS(results(dds_lrt), file = paste(directory_path, "/sc_zfpexp_cmint_lrt_results.rds", sep=""))
