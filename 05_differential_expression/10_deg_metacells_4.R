...
coldata <- adata_patient_125$obs[,c("Tumor_Site","Culture_Media", "ZFP_Expression", "Batch", "Patient")]
...
result_file <- "/data/chanjlab/CRC_ZFP36L2.092023/Organoid/output/deseq/metacell_ptn125_zfpexp_cmint_results.csv"

# Write the results table to a CSV file
write.csv(as.data.frame(res_1), file = result_file)

# Save the DESeqDataSet object
saveRDS(dds_1, file = "/data/chanjlab/CRC_ZFP36L2.092023/Organoid/output/deseq/metacell_ptn125_zfpexp_cmint_dds.rds")

# Save the DESeq results
saveRDS(results(dds_1), file = "/data/chanjlab/CRC_ZFP36L2.092023/Organoid/output/deseq/metacell_ptn125_zfpexp_cmint_results.rds")