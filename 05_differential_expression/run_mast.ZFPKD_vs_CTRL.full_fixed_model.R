...
df2 <- counts[cell_clusters == 'CTRL', ]
    ...
    Tumor_Site = c(Tumor_Site[cell_clusters == 'ZFP_KD'], Tumor_Site[cell_clusters == 'CTRL'])

    Culture_Media = factor(obs_df$Culture_Media, levels = c('HISC','BASE','Dedifferentiated'))
    Culture_Media = c(Tumor_Site[cell_clusters == 'ZFP_KD'], Tumor_Site[cell_clusters == 'CTRL'])
    ...
    colData(sca)$Culture_Media <- Culture_Media

    # Fit model
    formula = as.formula(paste('~ ', paste(colnames(colData(sca))[2:ncol(colData(sca))], collapse = '+'), ' + cond:Tumor_Site + cond:Culture_Media + Tumor_Site:Culture_Media'))
...
#    zlmCond <- zlm(~ cond + cngeneson, sca)
    ...
    # Export data
    write.csv(as.data.frame(fcHurdle), file.path(out_dir, sprintf("mast.ZFPKD_vs_CTRL.%s.adjust_replicate.csv", sample)), quote = FALSE)
