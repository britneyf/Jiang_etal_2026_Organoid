...
dds_1 <- DESeqDataSetFromMatrix(
  ...
  colData = coldata,
  design = ~ Tumor_Type + Culture_Media + ZFP_Expression + Patient
)
...
dds_1 <- DESeqDataSetFromMatrix(
  countData = count_matrix,
  colData = coldata,
  design = ~ Tumor_Site + Culture_Media + ZFP_Expression + Patient
)
...
res_1 <- results(dds_1, contrast = c("Patient", "125", "146"))
res_1

# Specify the file path where you want to save the results
result_file <- "/data/chanjlab/CRC_ZFP36L2.092023/Organoid/output/deseq/patientcontrast_125_vs_146_results.csv"

# Write the results table to a CSV file
write.csv(as.data.frame(res_1), file = result_file)

# Save the DESeqDataSet object
saveRDS(dds_1, file = "/data/chanjlab/CRC_ZFP36L2.092023/Organoid/output/deseq/patientcontrast_125_vs_146_dds.rds")

# Save the DESeq results
saveRDS(results(dds_1), file = "/data/chanjlab/CRC_ZFP36L2.092023/Organoid/output/deseq/patient_125_vs_146_results.rds")